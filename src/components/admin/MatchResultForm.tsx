...
export function MatchResultForm({
  ...
  matchId: number;
  onUnauthorized: () => void;
  onSaved?: (updated: MatchDetailResponse) => void;
  onCategoryLoaded?: (categoryId: number) => void;
}) {
  const { token } = useAuth();
  ...
  if (match.homeTeamId === null || match.awayTeamId === null || !homeTeam || !awayTeam) {
    return (
      <p className="py-12 text-center text-muted-foreground">
        Questo match del tabellone non è ancora giocabile: una delle due squadre non è stata determinata (in attesa
        del risultato di un turno precedente).
      </p>
    );
  }
